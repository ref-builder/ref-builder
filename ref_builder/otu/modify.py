...
def update_otu_identifiers(
    repo: Repo, otu: RepoOTU, taxid: int, ignore_cache: bool = False,
) -> RepoOTU | None:
    """Update OTU with new Taxonomy ID and associated name."""
    client = NCBIClient(ignore_cache)

    otu_logger = logger.bind(id=str(otu.id), original_taxid=otu.taxid, original_name=otu.name)
    ...
    with repo.use_transaction():
        repo.update_otu_identifiers(otu_id=otu.id, taxid=taxonomy.id, name=taxonomy.name)
